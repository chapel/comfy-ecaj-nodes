...
2. CODE: Review apps/ and packages/ for dead code, unresolved
   TODOs/FIXMEs, duplicate functions, commented-out code, debug
   artifacts, unused imports/exports

3. CONFIG: Review package.json, tsconfig, turbo.json, pnpm-workspace,
   and per-package configs for unused deps, misplaced deps (dev vs
   prod vs peer), broken scripts, stale configs, version drift

4. TESTS: Review test files for skipped tests, dead mocks, duplicate
...
**Docs:**
> Review documentation files for cruft in preparation for release. Look for: outdated content, stale TODOs, inconsistencies between docs, wrong command syntax, features documented that don't exist. Check README.md, AGENTS.md, CLAUDE.md, docs/ directory, and other .md files.

**Code:**
> Review apps/ and packages/ directories for code cruft. Look for: dead code, unused exports, unresolved TODO/FIXME comments, duplicate functions, commented-out code, debug artifacts, unused imports. Note file paths and line numbers.

**Config:**
> Review configuration files for cruft. Look for: unused dependencies, misplaced deps (devDeps vs deps vs peerDeps), broken npm scripts, stale tsconfig/turbo/eslint options, version inconsistencies across workspace packages. Check package.json files in root and each package, tsconfig.json, turbo.json, pnpm-workspace.yaml, and other config files.

**Tests:**
> Review test files for cruft. Look for: skipped tests (.skip/.todo), dead mock files, duplicate test names, flaky patterns, missing coverage, outdated fixtures.
