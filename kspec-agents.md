<!-- Generated by kspec v0.14.0 at 2026-06-23T23:10:33.336Z -->
...
- ComfyUI nodes use deferred execution: Entry/LoRA/Compose/Merge build immutable WIDEN recipe data only; Exit performs all tensor/GPU work.
- Architecture support is pluggable: add architecture-specific loaders/classifiers/auto-detection instead of branching inside generic executor paths.
- LoRA loaders must preserve exact base-model keyspace semantics, including fused/sliced weights and compound names; never silently drop unmatched LoRA groups.
- Tests should exercise parser, shape, block-classification, and executor behavior with CPU-safe synthetic tensors before any GPU/ComfyUI smoke.
- Keep ComfyUI integration thin: nodes expose recipe/config surfaces while lib/ owns algorithmic behavior and deterministic validation.
